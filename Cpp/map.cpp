...
// Given two coordinate pairs this function draws a line between them if they
// are on the screen
void draw_line(int32_t lon1, int32_t lat1, int32_t lon2, int32_t lat2){
  int32_t x1,x2,y1,y2;
  x1=longitude_to_x(current_map_num,lon1) - screen_map_x;
  x2=longitude_to_x(current_map_num,lon2) - screen_map_x;
  y1=latitude_to_y(current_map_num,lat1) - screen_map_y;
  y2=latitude_to_y(current_map_num,lat2) -  screen_map_y;
  if(is_on_screen(lon1,lat1) && is_on_screen(lon2,lat2)){
    tft.drawLine(x1,y1,x2,y2,ST7735_BLUE);
  }
}


// Determines whether or not a coordinate pair is on the screen.
bool is_on_screen(int32_t lon, int32_t lat){
  int32_t lon1,lat1,lon2,lat2,max_lon,min_lon,max_lat,min_lat;
  lon1=x_to_longitude(current_map_num,0+screen_map_x);
  lon2=x_to_longitude(current_map_num,display_window_width+screen_map_x);
  lat1=y_to_latitude(current_map_num,0+screen_map_y);
  lat2=y_to_latitude(current_map_num,display_window_height+screen_map_y);
